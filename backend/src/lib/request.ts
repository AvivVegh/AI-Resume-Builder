...
export const convertPathParametersToCamelCase = (event: APIGatewayEvent) => {
  ...
  if (event.pathParameters) {
    for (const key of Object.keys(event.pathParameters)) {
      pathParameters[lodash.camelCase(key)] = event.pathParameters[key];
    }
  }

  return pathParameters;
};
export const extractParams = <T = Record<string, any>>(
  event: APIGatewayEvent,
  props: string[],
) => {
  const convertedPathEvent = convertPathParametersToCamelCase(event);
  return lodash.pick(convertedPathEvent, props) as T;
};
...
export const extractQueryParams = <T>(
  ...
  if (Array.isArray(props)) {
    return lodash.pick<T>(queryParams as any, props) as T;
  }
  ...
  return lodash.pick<T>(mappedObj, Object.keys(desc)) as T;
};
 ...
 * a validation error.
 */
export const extractQueryParamJson = (
  event: APIGatewayEvent,
  paramName: string,
) => {
...
export const extractBody = <T = Record<string, any>>(
  event: APIGatewayEvent,
  props: string[],
) => {
  if (!event.body) {
  ...
  const body = JSON.parse(event.body);
  return lodash.pick(body, props) as T;
};
  ...
  if (object instanceof Array) {
    ...
    for (originalKey in object) {
      ...
      if (object.hasOwnProperty(originalKey)) {
        ...
        newKey = lodash.camelCase(originalKey);
        value = object[originalKey];
...
export const extractBodyFromFormData = (
  event: APIGatewayEvent,
  keys: string[],
) => {
  const parsed = querystring.decode(event.body);
  return lodash.pick(parsed, keys);
};
