...
export const toJoiObject = <T extends Joi.PartialSchemaMap<T>>(
  val?: T,
): Joi.ObjectSchema<T> => Joi.object({ ...val });
...
export type InterfaceFrom<T extends Joi.ObjectSchema<any>> =
  T extends Joi.ObjectSchema<infer U> ? ExpandRecursively<U> : never;
